...
func (s *Server) RegisterSubscriber(ctx context.Context, topic, queue string, disableAutoAck bool, handler broker2.Handler, binder broker2.Binder, opts ...broker2.SubscribeOption) error {
	s.Lock()
	defer s.Unlock()

	//var opts []broker.SubscribeOption
	opts = append(opts, broker2.WithQueueName(queue))
	if disableAutoAck {
		opts = append(opts, broker2.DisableAutoAck())
	}

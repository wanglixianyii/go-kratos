...
func newBroker(options broker2.Options) broker2.Broker {
...
func (r *rocketmqBroker) Options() broker2.Options {
	return r.opts
}

func (r *rocketmqBroker) Init(opts ...broker2.Option) error {
	r.opts.Apply(opts...)
...
func (r *rocketmqBroker) createConsumer(options *broker2.SubscribeOptions) (rocketmq.PushConsumer, error) {
	credentials := primitive.Credentials{
		AccessKey: r.accessKey,
		SecretKey: r.secretKey,
	}
...
func (r *rocketmqBroker) Publish(topic string, msg broker2.Any, opts ...broker2.PublishOption) error {
	buf, err := broker2.Marshal(r.opts.Codec, msg)
	if err != nil {
		return err
	}
...
func (r *rocketmqBroker) publish(topic string, msg []byte, opts ...broker2.PublishOption) error {
	options := broker2.PublishOptions{
...
func (r *rocketmqBroker) Subscribe(topic string, handler broker2.Handler, binder broker2.Binder, opts ...broker2.SubscribeOption) (broker2.Subscriber, error) {
	options := broker2.SubscribeOptions{
		Context: context.Background(),
		AutoAck: true,
		Queue:   r.groupName,
	}
	...
	if err := c.Subscribe(topic, consumer.MessageSelector{},
		func(ctx context.Context, msgs ...*primitive.MessageExt) (consumer.ConsumeResult, error) {
			...
			for _, msg := range msgs {
				...
				if err := broker2.Unmarshal(r.opts.Codec, msg.Body, &m.Body); err != nil {
					p.err = err
